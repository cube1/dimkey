...
impl OnnxTokenClassifier {
    ...
    pub fn try_load(model_dir: &Path) -> Result<Option<Self>, String> {
        ...
        let (tagging_scheme, label_map) = if config_path.exists() {
            ...
            let scheme = match config.tagging_scheme.to_lowercase().as_str() {
                "bio" => TaggingScheme::Bio,
                "ionly" | "i-only" | "i_only" => TaggingScheme::IOnly,
                other => return Err(format!("未知标注方案: {}", other)),
            };

            let mut lm = HashMap::new();
            for (entity, sensitive_name) in &config.label_map {
            ...
            let lm = Self::build_default_label_map(&id2label);
            (scheme, lm)
        };
...
impl NerBackend for OnnxTokenClassifier {
    fn detect_text(&mut self, text: &str) -> Result<Vec<RawEntity>, String> {
        ...
        // 解析输出并计算 label_ids / confidences（在块内完成以释放 outputs 借用）
        let (label_ids, confidences) = {
            ...
            (label_ids, confidences)
        };
        // outputs 在此处已释放，可以安全调用 &self 方法
        drop(outputs);

        // 根据标注方案分发后处理
        let items = match self.tagging_scheme {
...
mod tests {
    ...
    fn test_parse_sensitive_type() {
        ...
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("Title"), Some(SensitiveType::Title));
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("Email"), Some(SensitiveType::Email));
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("Phone"), Some(SensitiveType::Phone));
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("IdCard"), Some(SensitiveType::IdCard));
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("BankCard"), Some(SensitiveType::BankCard));
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("Ssn"), Some(SensitiveType::Ssn));
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("CreditCard"), Some(SensitiveType::CreditCard));
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("UsPhone"), Some(SensitiveType::UsPhone));
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("UkPhone"), Some(SensitiveType::UkPhone));
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("Passport"), Some(SensitiveType::Passport));
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("Iban"), Some(SensitiveType::Iban));
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("ZipCode"), Some(SensitiveType::ZipCode));
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("UkPostcode"), Some(SensitiveType::UkPostcode));
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("DriversLicense"), Some(SensitiveType::DriversLicense));
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("IpAddress"), Some(SensitiveType::IpAddress));

        // 未知类型应返回 None
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("Unknown"), None);
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type(""), None);
        assert_eq!(OnnxTokenClassifier::parse_sensitive_type("FooBar"), None);
    }
}
