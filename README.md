...
Dimkey 是一款本地文档脱敏工具，帮助你在使用外部 AI 工具前，快速清除文档中的敏感信息。拖入文件即可自动识别并脱敏，导出安全文件。**纯本地运行，零网络通信**。
...
```bash
# 克隆仓库
git clone https://github.com/cube1/dimkey.git
cd dimkey
...
- macOS (Apple Silicon) → `.dmg` 安装包（本地构建后手动上传）
- Windows (x86_64) → NSIS 安装包（CI 自动构建）
